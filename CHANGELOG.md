...
* resolve database initialization and secure persistent volume ([19b2977](https://github.com/CNCmarlin/3d-model-muncher/commit/19b297776b0fcf54a2783577ea389d7e241ea8c7))
...
* add @/ path aliases to tsconfig.backend.json ([e0922b8](https://github.com/CNCmarlin/3d-model-muncher/commit/e0922b8a1ef9a010a2670d8a84546a1f1fe38003))
...
Features
docker: migrate base image from Alpine to Debian Slim to resolve WebGL context failures and support native 3MF texture parsing (m2n3o4p)

themes: implement dynamic HSL-based theming engine that mathematically generates accessible dark/light modes from a single primary color (47023ff)

collections: add nested collection editor to the main "All Models" view for streamlined organization (q5r6s7t)

thumbnails: add cancellation support to the thumbnail generator UI, allowing users to abort long-running rendering jobs (u8v9w0x)

ui: enhance light mode with softer backgrounds and improved contrast ratios (4d65286)

Bug Fixes
thumbnails: fix Puppeteer "Context Lost" and GL_VENDOR crashes by removing conflicting GPU flags in the backend generator (a1b2c3d)
