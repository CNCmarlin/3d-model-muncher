...
class ProjectService {
    ...
    async updatePlateItemTransforms(transformsArray) {
        return await db.$transaction(
            transformsArray.map(t => db.buildPlateItem.update({
                where: { id: t.id },
                data: {
    ...
    async updateProjectItemQuantity(projectItemId, quantityDesired) {
        return await db.projectItem.update({
            where: { id: projectItemId },
            data: { quantityDesired }
        });
    }

    async updateProjectItemColor(projectItemId, colorHex) {
        return await db.projectItem.update({
            where: { id: projectItemId },
            data: { colorHex }
        });
    ...
    async updatePlateItemColor(plateItemId, colorHex) {
        return await db.buildPlateItem.update({
            where: { id: plateItemId },
            data: { colorHex }
        });
    }

    async cloneBuildPlateItem(plateItemId) {
        return await db.$transaction(async (tx) => {
            const source = await tx.buildPlateItem.findUnique({ where: { id: plateItemId } });
            if (!source) throw new Error("Source item not found");

            // Clone item with tiny spatial offset
            const cloned = await tx.buildPlateItem.create({
                data: {
                    buildPlateId: source.buildPlateId,
                    projectItemId: source.projectItemId,
                    quantity: 1,
                    positionX: (source.positionX || 0) + 15,
                    positionY: (source.positionY || 0) + 15,
            ...
            // Increment both assigned and desired since the user explicitly cloned it on the plate
            await tx.projectItem.update({
                where: { id: source.projectItemId },
                data: {
                    quantityAssigned: { increment: 1 },
                    quantityDesired: { increment: 1 }
                }
            });
