...
import { computeMD5, parse3MF as realParse3MF } from "./threeMFToJson";
 ...
 */
export function generate3MFJsonForDirectory_db(dir: string) {
 ...
 */
export function generateFileHash_db(modelUrl: string): Promise<string> {
 ...
 */
export function scanModelFile_db(model: Model): Promise<Partial<Model>> {
 ...
 */
export async function performHashCheck_db(models: Model[]): Promise<HashCheckResult> {
  let checkedFiles = 0;
  ...
  for (const model of models) {
    try {
      const hash = await generateFileHash_db(model.modelUrl);
  ...
  // Find duplicates
  const duplicateGroups = findDuplicates_db(updatedModels);
 ...
 */
export function findDuplicates_db(models: Model[]): DuplicateGroup[] {
  ...
  hashGroups.forEach((models, hash) => {
    if (models.length > 1) {
      ...
      const totalSize = formatFileSize_db(totalSizeBytes);
 ...
 */
export function formatFileSize_db(bytes: number): string {
 ...
 */
export async function scanModelDirectory_db(): Promise<Model[]> {
  return new Promise((resolve) => {
    setTimeout(() => {
 ...
 */
export function removeDuplicates_db(models: Model[], duplicateGroup: DuplicateGroup, keepModelId: string): Model[] {
 ...
 */
export function calculateSpaceSavings_db(duplicateGroups: DuplicateGroup[]): string {
  let totalSavings = 0;
  ...
  return formatFileSize_db(totalSavings);
}
