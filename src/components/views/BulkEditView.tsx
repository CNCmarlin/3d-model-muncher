import { BulkEditHelpDialog } from "@/components/bulk-edit/BulkEditHelpDialog";
...
import { BulkEditState, useBulkEditForm } from "@/hooks/bulk/useBulkEditForm";
...
export function BulkEditView({
    ...
    const GlobalEditsBar = () => {
        ...
        const removeGlobal = (field: keyof BulkEditState, value?: any, label?: string) => {
    ...
    const handleGlobalRemovalRequest = (field: keyof BulkEditState, value?: any, label?: string) => {
        if (models.length === 0) return;
        setConfirmRemoval({
            type: 'global',
    ...
    const handleSingleRemovalRequest = (id: string, field: string, value?: any) => {
        setConfirmRemoval({
            type: 'single',
            ids: [id],
            field: field as keyof BulkEditState,
            value,
            label: field
        });
    };
