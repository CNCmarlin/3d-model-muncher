...
export default function CollectionEditDrawer_DB({
  open,
  onOpenChange,
  collection,
  ...
  useEffect(() => {
    // This function runs when the component is destroyed (drawer unmounts)
    return () => {
      images.forEach(url => {
        // Only revoke URLs that were created locally (they start with 'blob:')
  ...
  const handleMassUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (collection?.id) {
      ...
      for (const file of files) {
        ...
        } catch (e) { console.error("[Drawer] Network error during mass upload:", e); }
      }
      ...
      if (newPaths.length > 0) {
        ...
        // Crucial: Notify parent with the new image list so the dashboard updates
        if (onSaved) {
          onSaved({ ...collection, images: nextImages });
        }
        toast.success(`Added ${newPaths.length} images to collection`);
      }
      setIsSaving(false);
    }
    // CASE B: Create Mode
      ...
      toast.info(`Added ${files.length} images to pending upload`);
    }
    e.target.value = '';
  };
  ...
  const handleDocumentUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!e.target.files || e.target.files.length === 0) return;
    const files = Array.from(e.target.files);
    console.log("[Drawer] Uploading documents:", files.length);
    ...
    if (collection?.id) {
      ...
      for (const file of files) {
        ...
        try {
          ...
          // [FIX] Check for JSON response to avoid silent crashes
          const contentType = res.headers.get("content-type");
          if (!contentType || !contentType.includes("application/json")) {
            const text = await res.text();
            console.error("[Drawer] Server Error (Not JSON):", text);
            toast.error(`Upload failed: Server error (${res.status})`);
            continue;
          }

          const data = await res.json();
          console.log("[Drawer] Doc upload response:", data);
    ...
    // CASE B: Create Mode (Pending)
      ...
      console.log("[Drawer] Added to pending:", files.length);
  ...
  useEffect(() => {
    // Logic to run when the drawer is closed or unmounted
    if (!open) {
      // 1. Memory Cleanup: Revoke any local preview blobs
      images.forEach(url => {
        if (url?.startsWith('blob:')) {
          URL.revokeObjectURL(url);
        }
  ...
  }, [open]); // Only triggers when the 'open' prop changes
