...
export function ModelCard_DB({
  ...
  // DB-First: Resolve 3D file URL from files relation, fall back to legacy flat fields
  const primaryFile = model.files?.find(f => f.isPrimary);
  const modelUrl = primaryFile?.filePath
    ? `/models/${primaryFile.filePath}`
    : (model.modelUrl || model.filePath);
  ...
  const neededWeightStr = meta.gcodeData?.totalFilamentWeight || model.filamentUsage?.toString();
    ...
    switch (fieldType) {
      case 'printTime':
        return {
          ...
          label: 'Print Time',
          value: model.printTime || meta.gcodeData?.printTime || ud.printTime || null
        };
      case 'filamentUsed':
        return {
          icon: <Weight className="w-3 h-3" />,
          label: 'Filament',
          value: model.filamentUsage || meta.gcodeData?.totalFilamentWeight || null
        };
      case 'fileSize': {
        // DB-First: Sum file sizes from the files relation
        const totalSize = model.files?.reduce((acc, f) => acc + (f.size || 0), 0);
        const sizeStr = totalSize ? `${(totalSize / 1024 / 1024).toFixed(1)} MB` : null;
        return {
