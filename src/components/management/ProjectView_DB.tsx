import { ImageWithFallback_DB as ImageWithFallback } from '@/components/common/ImageWithFallback_DB';
...
export function ProjectView({ collection, models, onUpdateCollection, onModelClick, onBack }: ProjectViewProps) {
    ...
    const handleDeletePlate = async (plateId: string) => {
        ...
        try {
            ...
            if (res.ok) {
                // Optimistic update
                const updatedPlates = collection.buildPlates?.filter((p: any) => p.id !== plateId) || [];
    ...
    const handleRenamePlate = async (plateId: string) => {
        ...
        try {
            ...
            const data = await res.json();
            if (data.success) {
                const updatedPlates = collection.buildPlates?.map((p: any) => p.id === plateId ? data.buildPlate : p) || [];
                onUpdateCollection({ ...collection, buildPlates: updatedPlates });
    ...
    const handleUpdateStatus = async (plateId: string, newStatus: string) => {
        try {
            ...
            if (res.ok) {
                ...
                const updatedPlates = collection.buildPlates?.map((p: any) =>
    ...
    const handleAssignModel = async (modelId: string, plateId: string) => {
        ...
        const target = currentPlates.find((p: any) => p.id === plateId);
        if (target) {
    ...
    return (
        <div className="flex h-full overflow-hidden bg-background">
            ...
            <div className="flex-1 flex flex-col min-w-0">
                ...
                <ScrollArea className="flex-1 p-6 bg-muted/10">
                    <div className="space-y-8 pb-20">
                        {collection.buildPlates?.map((plate: any) => (
                                    ...
                                    {plate.modelIds.length === 0 ? (
                                        ...
                                        <ScrollArea className="w-full whitespace-nowrap pb-2">
                                            <div className="flex gap-3">
                                                {plate.modelIds.map((id: Key | null | undefined) => {
            ...
            <div
                className={`bg-card border-l transition-all duration-300 ease-in-out flex flex-col ${isPoolOpen ? "w-80" : "w-0 opacity-0 overflow-hidden"
                    }`}
            >
                            ...
                            >
                                <div className="aspect-square relative">
                                    ...
                                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center gap-1 transition-opacity">
                                        ...
                                        {collection.buildPlates?.slice(0, 3).map((bp: any) => (
                                            <Button
                                                key={bp.id}
                                                size="sm"
                                                variant="secondary"
