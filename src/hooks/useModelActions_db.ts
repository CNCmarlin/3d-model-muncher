 ...
 */
export function useModelActions_db({
    models,
    setModels,
    filteredModels,
    setFilteredModels,
    ...
    const { updateModel, bulkUpdateModels } = useModelMutations_DB();
    ...
    const handleModelUpdate = async (updatedModel: Model) => {
        ...
        updateModel.mutate(
            {
                id: updatedModel.id,
                data: updatedModel as any, // Cast to any to bypass strict OMits since backend handles partials
            },
            {
                onSuccess: () => {
