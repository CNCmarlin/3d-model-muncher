...
export function useBulkEditForm({ models, selectedTargetIds, isOpen }: UseBulkEditFormProps) {
    ...
    const updateStaged = (key: keyof BulkEditState, value: any) => {
        setStagedEdits(prev => {
            const next = { ...prev };
    ...
    const removeStaged = (key: keyof BulkEditState) => {
        setStagedEdits(prev => {
            const next = { ...prev };
            selectedTargetIds.forEach(id => {
                if (next[id]) {
    ...
    const handleFieldToggle = (field: keyof FieldSelection) => {
        ...
        if (fieldSelection[field]) {
            ...
            const stateKey = field as keyof BulkEditState; // Works for most
    ...
    const getCommonValues = () => {
        ...
        const getEffectiveValue = (id: string, field: keyof BulkEditState) => {
        ...
        const checkField = (field: keyof BulkEditState) => {
    ...
    const editState = getCommonValues() as BulkEditState;
...
export type UseBulkEditFormResult = ReturnType<typeof useBulkEditForm>;
